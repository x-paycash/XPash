    ...
    public function ajax() //don't use default dataTable() method
    {
        return datatables()
            ...
            ->addColumn('action', function ($backup)
            {
                ...
                $edit = (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_database_backup')) ? '<a href="' . url(Config::get('adminPrefix').'/backup/download/' . $backup->id) . '" class="btn btn-xs btn-primary"><i class="fa fa-download"></i></a>' : '';
                return $edit;
