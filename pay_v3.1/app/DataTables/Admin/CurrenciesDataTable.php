    ...
    public function ajax() //don't use default dataTable() method
    {
        return datatables()
            ...
            ->addColumn('action', function ($currency)
            {
                ...
                $edit = (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_currency')) ? '<a href="' . url(Config::get('adminPrefix').'/settings/edit_currency/' . encrypt($currency->id)) . '" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;' : '';
                $delete = (Common::has_permission(\Auth::guard('admin')->user()->id, 'delete_currency')) ? '<a href="' . url(Config::get('adminPrefix').'/settings/delete_currency/' . encrypt($currency->id)) . '" class="btn btn-xs btn-danger delete-warning"><i class="glyphicon glyphicon-trash"></i></a>&nbsp;' : '';
                ...
                if ($currency->type != 'crypto')
                {
                    ...
                    $pm = (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_currency')) && $currency->status == 'Active'? '<a href="' . url(Config::get('adminPrefix').'/settings/payment-methods/stripe/' . $currency->id) . '" class="btn btn-xs btn-primary">
