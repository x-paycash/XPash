    ...
    public function ajax()
    {
        ...
        return datatables()
            ...
            ->editColumn('user_id', function ($exchange)
            {
                ...
                $senderWithLink = (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_user')) ? '<a href="' . url(Config::get('adminPrefix').'/users/edit/' . $exchange->user_id) . '">'.$sender.'</a>' : $sender;
            ...
            ->addColumn('action', function ($exchange)
            {
                ...
                '<a href="' . url(Config::get('adminPrefix').'/exchange/edit/' . $exchange->id) . '" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;' : '';
                return $edit;
            })
