    ...
    public function ajax()
    {
        return datatables()
            ...
            ->addColumn('user', function ($merchant)
            {
                ...
                $userWithLink = (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_user')) ? '<a href="' . url(Config::get('adminPrefix').'/users/edit/' . $merchant->user->id) . '">'.$user.'</a>' : $user;
                return $userWithLink;
            })
            ...
            ->addColumn('action', function ($merchant)
            {
                ...
                '<a href="' . url(Config::get('adminPrefix').'/merchant/edit/' . $merchant->id) . '" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;' : '';
                return $edit;
            })
