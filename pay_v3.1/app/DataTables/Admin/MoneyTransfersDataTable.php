    ...
    public function ajax()
    {
        return datatables()
            ...
            ->addColumn('sender', function ($transfer)
            {
                ...
                $senderWithLink = (Common::has_permission(\Auth::guard('admin')->user()->id, 'edit_user')) ? '<a href="' . url(Config::get('adminPrefix').'/users/edit/' . $transfer->sender->id) . '">' . $sender . '</a>' : $sender;
            ...
            ->addColumn('action', function ($transfer)
            {
                ...
                '<a href="' . url(Config::get('adminPrefix').'/transfers/edit/' . $transfer->id) . '" class="btn btn-xs btn-primary"><i class="glyphicon glyphicon-edit"></i></a>&nbsp;' : '';
                return $edit;
            })
