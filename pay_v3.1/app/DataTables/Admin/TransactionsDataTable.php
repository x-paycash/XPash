    ...
    public function ajax()
    {
        ...
        return datatables()
           ...
           ->addColumn('receiver', function ($transaction)
            ...
            {
                return ($transaction->currency->type != 'fiat') ? number_format($transaction->subtotal, 8, '.', '') : formatNumber($transaction->subtotal);
            })
            ->addColumn('fees', function ($transaction)
            {
                return (($transaction->charge_percentage == 0) && ($transaction->charge_fixed == 0) ? '-' : (($transaction->currency->type != 'fiat') ? number_format(($transaction->charge_percentage + $transaction->charge_fixed), 8, '.', ' ') : formatNumber($transaction->charge_percentage + $transaction->charge_fixed)));
            })
            ->editColumn('total', function ($transaction)
            {
                if ($transaction->total > 0)
                {
                    ...
                    {
                        if ($transaction->transaction_type->name == 'Crypto_Exchange_To') {
                            $total = '<td><span class="text-green">+' . number_format($transaction->total, 8, '.', '') . '</span></td>';
                        } else {
                ...
                {
                    if ($transaction->currency->type != 'fiat' && ($transaction->transaction_type->name == 'Crypto_Sent' || $transaction->transaction_type->name == 'Crypto_Exchange_From'))
                    {
                        if (!empty($transaction->cryptoapi_log->payload))
                        {
                            $payloadJson = json_decode($transaction->cryptoapi_log->payload, true);
                            $network_fee =  isset($payloadJson['network_fee']) ? $payloadJson['network_fee'] : 0.00000000;
                        }
                        $total = '<td><span class="text-red">' . (number_format($transaction->total - $network_fee, 8, '.', '')) . '</span></td>';
                    }
                    else
                    {
                         if ($transaction->transaction_type->name == 'Fiat_Exchange_From') {
                            $total = '<td><span class="text-red">' . number_format($transaction->total, 8, '.', '') . '</span></td>';
                        } else {
