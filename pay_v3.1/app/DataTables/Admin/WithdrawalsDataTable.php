...
class WithdrawalsDataTable extends DataTable
{
    ...
    public function ajax()
    {
        return datatables()
            ...
            {
                if ($withdrawal->payment_method->name == "Paypal")
                {
                    $data =  !empty($withdrawal->payment_method_info) ? $withdrawal->payment_method_info : '-';
                }
                elseif ($withdrawal->payment_method->name == "MobileMoney") 
                {
                    $data = !empty($withdrawal->withdrawal_detail) ? $withdrawal->payment_method_info.' '.'('.('*****'.substr($withdrawal->withdrawal_detail->mobile_number,-4)).')' : '-';
                }
                else
                {
