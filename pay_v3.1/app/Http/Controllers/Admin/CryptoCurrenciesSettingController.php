...
class CryptoCurrenciesSettingController extends Controller
{
    ...
    public function updateBlockIoSettings(Request $request)
    {
        ...
        if (checkDemoEnvironment() == true)
        {
            ...
            return redirect(Config::get('adminPrefix')."/settings/crypto-currencies-settings/$network");
        }
    ...
    /**
     * Get coin credentials
     * @param  string $network 
     * @return json          
     */
    public function coinCredentials($network = 'BTC')
    {
        $coinCredentials = $this->cryptoCurrency->getCryptoCurrenciesSetting($network, 'All', ['*']);
        if (is_null($coinCredentials)) {
            return response()->json([
                'status' => 400
            ]);
        }
