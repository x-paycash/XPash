...
class UserController extends Controller
{
    ...
    public function eachUserDepositSuccess(Request $request)
    {
        ...
        catch (Exception $e)
        {
            ...
            Session::forget('transInfo');
            clearActionSession();
            $this->helper->one_time_message('error', 'Money deposited successfully, but mail or sms couldn\'t sent.');
            return redirect(Config::get('adminPrefix')."/users/deposit/create/$user_id");
        }
    }
...
/* Start of Admin Crypto Send */
    public function eachUserCryptoSend($id, Request $request)
    {
        ...
        $data['activeCryptoCurrencies'] = $this->cryptoCurrency->getActiveCryptoCurrencies();
        ...
        if ($request->isMethod('post'))
        {
            ...
            if ($res['status'] == 401)
            {
                ...
                return redirect(Config::get('adminPrefix').'/users/crypto/send/' . $request->user_id);
            }
    ...
    public function eachUserCryptoSendSuccess(Request $request)
    {
        ...
        {
            $this->helper->one_time_message('error', $res['message']);
            return redirect(Config::get('adminPrefix').'/users/crypto/send/' . $res['user_id']);
        }
...
/* Start of Admin Crypto Receive */
    public function eachUserCryptoReceive($id, Request $request)
    {
        ...
        if ($request->isMethod('post'))
        {
            ...
            {
                $this->helper->one_time_message('error', $res['message']);
                return redirect(Config::get('adminPrefix').'/users/crypto/receive/' . $request->user_id);
            }
            //for confirm page only
            $data['cryptoTrx'] = $res['cryptoTrx'];
    ...
    public function eachUserCryptoReceiveSuccess(Request $request)
    {
        ...
        {
            $this->helper->one_time_message('error', $res['message']);
            return redirect(Config::get('adminPrefix').'/users/crypto/receive/' . $res['user_id']);
        }
...
// Extended Functions (Crypto Send & Receive)- ends
    ...
    public function eachUserTransaction($id, EachUserTransactionsDataTable $dataTable)
    {
        ...
        $data['t_status']   = $eachUserTransactions->select('status')->groupBy('status')->get();
        $data['t_currency'] = $eachUserTransactions->select('currency_id')->groupBy('currency_id')->get();
        $data['t_type']     = $eachUserTransactions->select('transaction_type_id')->groupBy('transaction_type_id')->get();
    ...
    /**
     * Method eachUserReferralAwards
     *
     * @param $id $id [explicite description]
     * @param EachUserReferralAwardsDataTable $dataTable [explicite description]
     *
     * @return void
     */
