...
class PayoutMoneyController extends Controller
{
    ...
    {
        $paymentMethod = PayoutSetting::where(['user_id' => request('user_id')])->get(['id', 'user_id', 'type', 'email', 'account_name', 'mobile_number']);
        $pm            = [];
        for ($i = 0; $i < count($paymentMethod); $i++)
        {
            ...
            if ($paymentMethod[$i]->email) {
                $pm[$i]['paymentMethodCredential'] = $paymentMethod[$i]->email;
            } else if ($paymentMethod[$i]->mobile_number) {
                $pm[$i]['paymentMethodCredential'] = $paymentMethod[$i]->mobile_number;
            } else {
    ...
    public function getWithdrawDetailsWithAmountLimitCheck()
    {
        ...
        $payoutSetting             = PayoutSetting::with(['paymentMethod:id,name'])->where(['id' => $payoutSettingId])->first(['account_name', 'account_number', 'type', 'swift_code', 'bank_name', 'mobilemoney_id', 'mobile_number']);
        ...
        $success['mobilemoney_id'] = $payoutSetting->mobilemoney_id;
        $success['mobile_number']  = $payoutSetting->mobile_number;
        if (!is_null($payoutSetting->mobilemoney_id)) {
            $mobilemoney = MobileMoney::find($payoutSetting->mobilemoney_id);
            $success['mobilemoney_name'] = $mobilemoney->mobilemoney_name;
        }
