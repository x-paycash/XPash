...
class PayoutSettingController extends Controller
{
    ...
    public function index()
    {
        ...
        $payoutSettings = PayoutSetting::with(['paymentMethod:id,name', 'mobilemoney'])
    ...
    public function store()
    {
        try {
            ...
            } else if ($paymentMethodType == 11) {
                $payoutSetting->mobilemoney_id      = request('vendor');
    ...
    {
        $paymentMethods    = PaymentMethod::whereNotIn('name', ['Mts', 'Stripe', '2Checkout', 'PayUMoney', 'Coinpayments', 'Payeer','BlockIo', 'Flexpay'])
            ->where(['status' => 'Active'])
            ->get(['id', 'name']);
    ...
    {
        $networks= MobileMoney::get(['id','mobilemoney_name']);
        $success['status']  = $this->successStatus;
        return response()->json(['success' => $success, 'networks' => $networks], $this->successStatus);
    }
}
