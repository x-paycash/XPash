    	...
    	if ($notifArray['type'] == 'address')
    	{
	        ...
	        if (count($cryptoCurrenciesSetting) > 0 )
	        {
	            ...
	            if ($responseConfirmations == 1)
	            {
	            	...
	            	{
                   		$getCryptoSentApiLogSql = $getCryptoSentApiLog = CryptoapiLog::with(['transaction:id,user_id,end_user_id,currency_id,uuid'])->where(['network' => $responseNetwork])->whereIn('object_type', ['crypto_sent', 'crypto_exchange_from', 'fiat_exchange_from'])->whereRaw('payload REGEXP ?', ['[[:<:]]' . $responseTxid . '[[:>:]]'])->get();
	            ...
	            {
	            	$getCryptoSentReceivedApiLogsSql = $getCryptoSentReceivedApiLogs = CryptoapiLog::where(['confirmations' => 1, 'network' => $responseNetwork])->whereIn('object_type', ['crypto_sent', 'crypto_received', 'crypto_exchange_from', 'crypto_exchange_to', 'fiat_exchange_from', 'fiat_exchange_to'])->whereRaw('payload REGEXP ?', ['[[:<:]]' . $responseTxid . '[[:>:]]'])->get();
