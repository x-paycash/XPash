...
class DepositController extends Controller
{
    ...
    public function getDepositMatchedFeesLimitsCurrencyPaymentMethodsSettingsPaymentMethods(Request $request)
    {
        $feesLimits = FeesLimit::with([
            ...
            {
                $q->where(['status' => 'Active'])->where('id', '!=', 10);
            },
        ])
    ...
    public function confirmFlexpay()
    {
        \Log::channel('daily')->info(request()->isMethod('post'));
        \Log::channel('daily')->info(request()->all());
        if (!empty(request()->reference) && request()->status == 0) {
            $transaction = Transaction::where(['uuid' => request()->reference])->first();

            $deposit = Deposit::where(['id' => $transaction->transaction_reference_id, 'uuid' => request()->reference])->first();
            $deposit->status = 'Success';
            $deposit->save();

            $wallet = Wallet::where(['currency_id' => $transaction->currency_id, 'user_id' => $transaction->user_id])->first();
            if (empty($wallet)) {
                $wallet = new Wallet;
            ...
            if (config('referral.is_active')) {
                ...
                $refAwardData                    = [];
                $refAwardData['userId']          = $transaction->user_id;
                $refAwardData['currencyId']      = $transaction->currency_id;
                $refAwardData['currencyCode']    = $currencyCode;
