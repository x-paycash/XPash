...
class MerchantController extends Controller
{
    ...
    public function index()
    {
        ...
        $data['icon']          = 'user';
        $data['list']          = Merchant::with(['appInfo', 'currency:id,code'])->where(['user_id' => Auth::user()->id])->orderBy('id', 'desc')->paginate(10);

        $data['defaultWallet'] = Wallet::where(['user_id' => auth()->user()->id, 'is_default' => 'Yes'])->first(['currency_id']); //new
