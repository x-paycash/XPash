                                    ...
                                    <ul>
                                        <li>
                                            @if(empty($wallet->currency->logo))
                                                <img src="{{theme_asset('public/user_dashboard/images/favicon.png')}}" class="img-responsive">
                                            @else
                                            ...
                                            @if($wallet->balance > 0)
                                                @if ($wallet->currency->type != 'fiat')
                                                    <p class="text-success">{{ moneyFormat($wallet->currency->code, '+'.formatNumber($wallet->balance)) }} <i class="material-icons sync-crypto-balance" data-wallet-Id="{{ $walletId }}"
                                                        data-wallet-currency-code="{{ $walletCurrencyCode }}">{{ __('sync') }}</i></p>
                                                @else
                                                    <p class="text-success">{{ moneyFormat($wallet->currency->code, '+'.formatNumber($wallet->balance)) }}</p>
                                                @endif
                                            @elseif($wallet->balance == 0)
                                                @if ($wallet->currency->type != 'fiat')
                                                    <p>{{ moneyFormat($wallet->currency->code,formatNumber($wallet->balance)) }} <i class="material-icons sync-crypto-balance" data-wallet-Id="{{ $walletId }}" data-wallet-currency-code="{{ $walletCurrencyCode }}">{{ __('sync') }}</i></p>
                                                @else
                                                    <p>{{ moneyFormat($wallet->currency->code,formatNumber($wallet->balance)) }}</p>
                                                @endif
                                            @elseif($wallet->balance < 0)
                                                @if ($wallet->currency->type != 'fiat')
                                                    <p class="text-danger">{{ moneyFormat($wallet->currency->code,formatNumber($wallet->balance)) }} <i class="material-icons sync-crypto-balance" data-wallet-Id="{{ $walletId }}" data-wallet-currency-code="{{ $walletCurrencyCode }}">{{ __('sync') }}</i></p>
                                                @else
                                        ...
                                        <div class="row pb6">
                                            @if ($wallet->currency->type != 'fiat')
                                                <a href="{{ url("/crpto/send/".$walletCurrencyCode."/".$walletId) }}" class="btn btn-cust-crypto left">{{ __('Send') }}</a>
                                                <a href="{{ url("/crpto/receive/".$walletCurrencyCode."/".$walletId) }}" class="btn btn-cust-crypto right">{{ __('Receive') }}</a>
                                            @endif
                                        </div>
                        ...
                        <div class="card-footer">
                            <div class="dash-btn row pb6">
                                <div class="left col-md-8 pb6">
                                    <small class="form-text text-muted"><strong>*{{ __('Fiat Currencies Only') }}</strong></small>
                                </div>
                            </div>
                                ...
                                @if(Common::has_permission(auth()->id(),'manage_deposit'))
                                    <div class="left col-md-6 pb6">
                                        <a href="{{url('deposit')}}" class="btn btn-cust col-md-12">
                                            <img src="{{theme_asset('public/user_dashboard/images/deposit.png')}}"
                                ...
                                @if(Common::has_permission(auth()->id(),'manage_withdrawal'))
                                    <div class="right col-md-6">
                                        <a href="{{url('payouts')}}" class="btn btn-cust col-md-12 ">
                                            <img src="{{theme_asset('public/user_dashboard/images/withdrawal.png')}}" class="img-responsive"> &nbsp;@lang('message.dashboard.button.payout')
                                        </a>
                                    </div>
                                @endif
                            </div>
                                ...
                                @if(Common::has_permission(auth()->id(),'manage_exchange'))
                                    <div class="center col-md-6">
                                        <a href="{{url('exchange')}}" class="btn btn-cust col-md-12">
                                            <img src="{{theme_asset('public/user_dashboard/images/exchange.png')}}" class="img-responsive" style="margin-top:3px;">
