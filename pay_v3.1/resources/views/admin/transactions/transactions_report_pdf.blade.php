                ...
                <table style="width:100%; border-radius:1px; border-collapse: collapse; text-align:center;"> <!-- Add Text Aligned here -->
                    ...
                    @foreach($transactions as $transaction)
                        <tr style="background-color:#fff; text-align:center; font-size:12px; font-weight:normal;">
                            ...
                            @else
                                @if ($transaction->transaction_type->name == 'Crypto_Exchange_To' || $transaction->transaction_type->name == 'Fiat_Exchange_From')
                                    <td>{{ number_format($transaction->subtotal, 8, '.', '') }}</td>
                                @else
                            ...
                            @if ($transaction->total > 0)
                                ...
                                @else
                                    @if ($transaction->transaction_type->name == 'Crypto_Exchange_To')
                                        <td>{{ '+' . number_format($transaction->total, 8, '.', '')}}</td>
                                    @else
                            ...
                            @else
                                @if ($transaction->currency->type != 'fiat')
                                    @if ($transaction->transaction_type->name == 'Crypto_Sent' || $transaction->transaction_type->name == 'Crypto_Exchange_From')
                                        <td>{{ number_format(($transaction->total - json_decode($transaction->cryptoapi_log->payload)->network_fee), 8, '.', '') }}</td>
                                    @else
                                        <td>{{ $transaction->total }}</td>
                                    @endif
                                @else
