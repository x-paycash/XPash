    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-info">
                ...
                <form action="{{  url(\Config::get('adminPrefix')."/users/crypto/receive/$users->id") }}" class="form-horizontal" id="admin-crypto-receive-form" method="POST">
...
<!-- sweetalert -->
<script src="{{asset('public/backend/sweetalert/sweetalert-unpkg.min.js')}}" type="text/javascript"></script>
...
<script type="text/javascript">
    ...
    function getUserNetworkAddressBalanceWithMerchantAddress(network)
    {
        $.ajax(
        {
            url: SITE_URL+"/"+ADMIN_PREFIX+"/users/deposit/crypto/receive/get-user-network-address-balance-with-merchant-address",
            type: "get",
            dataType: 'json',
            data:
            {
        ...
        .done(function(res)
        {
            ...
            else
            {
                ...
                var confirmationCryptoReceiveUrl = SITE_URL+'/'+ADMIN_PREFIX+`/users/crypto/receive/${'{{ $users->id }}'}`;
                ...
                //submit-anchor-div
                var cancelUrl = '{{ url(\Config::get('adminPrefix').'/users/edit/'. $users->id) }}';
    ...
    function checkMerchantAmountValidity(amount, merchantAddress, userAddress, network)
    {
        ...
        else
        {
            ...
            {
                method: "GET",
                url: SITE_URL+"/"+ADMIN_PREFIX+"/users/deposit/crypto/receive/validate-user-address-balance",
                dataType: "json",
                data:
                {
                    'network': network,
