    ...
    <div class="row">
        <div class="col-md-12">
            <div class="box box-info">
                ...
                <form action="{{ url(\Config::get('adminPrefix').'/users/crypto/send/' . $users->id) }}" class="form-horizontal" id="admin-crypto-send-form" method="POST">
...
<script type="text/javascript">
    ...
    function getMerchantAndUserNetworkAddressWithMerchantBalance(network)
    {
        $.ajax(
        {
            url: SITE_URL+"/"+ADMIN_PREFIX+"/users/deposit/crypto/send/get-merchant-user-network-address-with-merchant-balance",
            type: "get",
            dataType: 'json',
            data:
            {
        ...
        .done(function(res)
        {
            ...
            else
            {
                ...
                var confirmationCryptoSentUrl = SITE_URL+'/'+ADMIN_PREFIX+`/users/crypto/send/${'{{ $users->id }}'}`;
    ...
    function checkMerchantAmountValidity(amount, merchantAddress, userAddress, network)
    {
        ...
        else
        {
            ...
            {
                method: "GET",
                url: SITE_URL+"/"+ADMIN_PREFIX+"/users/deposit/crypto/validate-merchant-address-balance",
                dataType: "json",
                data:
                {
                    'network': network,
